...
function StatCard({ icon: Icon, label, value, subValue, color, link }: {
  ...
  return (
    <Link to={link ?? '#'}>
      <motion.div
        whileHover={{ y: -2 }}
        className="p-5 rounded-2xl bg-bg-card border border-border hover:border-border-hover transition-all group"
      >
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/pomodoro"
            className="flex items-center gap-2 px-4 py-2.5 bg-primary hover:bg-primary-hover text-white rounded-xl text-sm font-medium transition-all shadow-glow"
          >
            <Timer size={16} />
            Start Focus
          </Link>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Clock} label="Today's Study" value={formatMinutes(todayMinutes)} subValue="Keep going!" color="#4F8CFF" link="/pomodoro" />
        <StatCard icon={CheckSquare} label="Tasks Today" value={`${completedTasks}/${totalTasks}`} subValue={`${completionPct}% complete`} color="#22C55E" link="/tasks" />
        <StatCard icon={Flame} label="Current Streak" value={`${streak} days`} color="#F59E0B" link="/habits" />
        <StatCard icon={TrendingUp} label="Focus Score" value={`${todayFocusScore}%`} color="#7C5CFF" link="/analytics" />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="minutes" stroke="#4F8CFF" strokeWidth={2} fill="url(#studyGradient)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <ProgressBar value={getLevelProgress(state.user?.xp ?? 0)} color="#7C5CFF" size="sm" />
          ...
          {todayTasks.length === 0 ? (
            ...
            <div className="space-y-2">
              {todayTasks.slice(0, 5).map(task => (
                <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-bg-elevated transition-colors">
          ...
          {upcomingDeadlines.length === 0 ? (
            ...
            <div className="space-y-2">
              {upcomingDeadlines.map(event => (
                <div key={event.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-bg-elevated transition-colors">
        ...
        <Card>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {subjects.slice(0, 6).map(subject => (
              <div key={subject.id} className="p-3 rounded-xl bg-bg-elevated border border-border">
                <div className="flex items-center gap-2 mb-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { to: '/pomodoro', icon: Timer, label: 'Pomodoro', color: '#4F8CFF' },
          { to: '/tasks', icon: CheckSquare, label: 'Tasks', color: '#22C55E' },
          { to: '/notes', icon: BookOpen, label: 'Notes', color: '#7C5CFF' },
          { to: '/habits', icon: Target, label: 'Habits', color: '#F59E0B' },
        ].map(action => (
          <Link key={action.to} to={action.to}>
            <motion.div
              whileHover={{ y: -2 }}
              className="flex items-center gap-3 p-4 rounded-2xl bg-bg-card border border-border hover:border-border-hover transition-all"
            >
